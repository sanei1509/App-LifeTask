...
const todoCreatorModal = new bootstrap.Modal(
  document.getElementById("todo-creator-modal")
);
...
function fetchToDos() {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      for (
        let taskIndex = 0;
        taskIndex < numberOfTasksToGenerate;
        taskIndex++
      ) {
        const subTasks = [];
        const files = [];
...
function renderSubTasks(todo, subTaskContainerElement, subTaskTemplate) {
  ...
  if (todo.subTasks.length > 0) {
    for (
      ...
      const subTaskCheckbox = subTaskClone.querySelector(
        ".list-group-item > div > input:first-child"
      );
      const subTaskTitle = subTaskClone.querySelector(
        ".list-group-item > div > input:last-child"
      );
      const subTaskDeleteButton = subTaskClone.querySelector(
        ".list-group-item > .btn-delete-sub-task"
      );
...
function openViewTaskModal(todo) {
  ...
  const filesContainerElement = modalElement.querySelector(
